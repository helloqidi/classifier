...
module Classifier
  ...
  class Bayes
    ...
    def initialize(*categories)
      #判断是否有redis
      if_hash_redis = categories.last
      if if_hash_redis.class == Hash && ( if_hash_redis[:redis]!=nil)
        @redis = if_hash_redis[:redis]
        categories.pop
      else
        @redis = nil
      end

      @name = categories.collect{|x| x.prepare_category_name_to_s}.join("_")
      @storage =  Hash.new
      @key_name = "Bayes_#{@name}"

      #判断是否从redis初始化
      if @redis != nil && @redis.exists(@key_name)
        load_state
      else
        @categories = Hash.new
    ...
    def load_state
      ...
      @categories = @storage[@key_name][:categories]
      @total_words = @storage[@key_name][:total_words]
      @category_counts = @storage[@key_name][:category_counts]
    end
